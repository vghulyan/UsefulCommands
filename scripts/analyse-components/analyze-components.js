...
<label>📂 Filter by File: <select id="fileFilter"><option value="">-- All Files --</option>`;
          ...
          Object.values(usageMap).forEach((files) =>
            Object.keys(files).forEach((f) => allFiles.add(f))
          );
          Array.from(allFiles)
            .sort()
            .forEach((f) => {
...
<td>${component}</td><td>${file}</td><td>${count}</td><td>${total}</td>
          ...
          const unusedComponents = Array.from(declaredComponents).filter(
            (x) => !usedComponents.has(x)
          );
